...
18. regimen_support_infoのフォーマット規則（非常に重要: 省略・簡略化は厳禁。誰が見ても実務で使えるレベルの具体的かつ詳細な数値を記載すること）:
    ...
    - indications: 併用レジメンの場合は、薬剤ごとのバラバラな適応ではなく、そのレジメンとして認められている日本での正確な保険適応病名（例: 「治癒切除不能な進行・再発の胃癌」「HER2陽性転移性乳癌」等）を記載すること。
    ...
    - references: 決して省略してはならない。バンクーバー形式で必ず番号付きで記載すること。例: 「1. 著者. タイトル. 雑誌. 年;巻(号):頁.\n2. ...」
